...
from candrel.ep300_crebbp_replication import (
    ANALYSIS_EXPOSED,
    ANALYSIS_REFERENCE,
    Context,
    T0Stop,
...
def test_matrix_parse_stop_is_matrix_coverage():
    assert classify_context_stop(ValueError("non-numeric value for ('m1', 'EP300 (2033)')")) == "T0_MATRIX_COVERAGE"


def test_manifest_binds_pre_endpoint_implementation_boundary():
    manifest = verify_implementation_boundary(Path("experiments/EXP-20260822-022/manifest.json"))
