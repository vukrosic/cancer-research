...
from candrel.kmt2d_kmt2c_replication import (
    ANALYSIS_EXPOSED,
    ANALYSIS_REFERENCE,
    Context,
    T0Stop,
...
def test_matrix_parse_stop_is_matrix_coverage():
    assert classify_context_stop(ValueError("non-numeric value for ('m1', 'KMT2D (8085)')")) == "T0_MATRIX_COVERAGE"


def test_manifest_binds_pre_endpoint_implementation_boundary():
    manifest = verify_implementation_boundary(Path("experiments/EXP-20260822-024/manifest.json"))
