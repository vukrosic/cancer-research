...
EXPERIMENT_ID = "EXP-20260822-018"
PAIR_ID = "CDKN2A_damaging_to_TYMS"
STATUS_COLUMN = "CDKN2A (1029)"
TARGET_COLUMN = "TYMS (7298)"
SOURCES = ("Avana", "KY")
LOADER_EXPOSED = "matrix_intact"
LOADER_REFERENCE = "damaging"
ANALYSIS_EXPOSED = "damaging"
ANALYSIS_REFERENCE = "matrix_intact"
EXPECTED_MODULE_PATH = "src/candrel/cdkn2a_tyms_replication.py"
...
EXPECTED_STATUS_COUNTS = {"Avana": {ANALYSIS_EXPOSED: 110, ANALYSIS_REFERENCE: 865}, "KY": {ANALYSIS_EXPOSED: 37, ANALYSIS_REFERENCE: 278}}
EXPECTED_MIXED_LINEAGES = {"Avana": 19, "KY": 11}
DESIGN_SEEDS = {"Avana": 20261800, "KY": 20261900}
INFERENCE_SEEDS = {"Avana": 20271800, "KY": 20271900}
EXPECTED_ROSTER_SHA256 = "df50a72ac86b161e16ebc5a2eb2b2f5c8d35151d94da4046c375f5ab0f603bb5"
EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256 = "528294bb9cbe70081edc3d05e985bd992ea0c605656c283b5a4ed53794b97402"
...
def verify_implementation_boundary(manifest_path: Path) -> dict[str, object]:
    try:
        ...
        if manifest["experiment_id"] != EXPERIMENT_ID or manifest["entrypoint"] != "uv run candrel-cdkn2a-tyms-replication":
        ...
        if sha256(census) != manifest["candidate_census"]["sha256"] or sha256(receipt) != manifest["design_receipt"]["sha256"]:
            raise ValueError("census/design receipt SHA-256 drift")
        ...
        expected_design = {"mean_shift": engine.NORMAL_MEAN_SHIFT, "null_permutations": engine.PERMUTATIONS, "alternative_simulations": engine.DESIGN_SIMULATIONS, "planning_power_seeds": DESIGN_SEEDS, "expected_critical_delta": {"Avana": -0.11511227999190775, "KY": -0.19230769230769232}, "expected_power": {"Avana": 0.8954, "KY": 0.5364}, "minimum_power_for_confirmatory_label": engine.MIN_CONFIRMATORY_POWER, "frozen_label": "FEASIBILITY_ONLY"}
        ...
        if boundary["implementation_module"] != EXPECTED_MODULE_PATH or boundary["engine_module"] != ENGINE_MODULE_PATH or boundary["project_file"] != PROJECT_PATH:
        ...
        expected_modules = {EXPECTED_MODULE_PATH: None, ENGINE_MODULE_PATH: None, PROJECT_PATH: None}
        if {item["path"] for item in boundary["modules"]} != set(expected_modules):
...
def write_context_ledger(path: Path, contexts: Sequence[Context]) -> None:
    fields = ["source", "model_id", "lineage", "eligible_screen_ids", "CDKN2A_matrix_value", "CDKN2A_status"]
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for c in contexts:
            writer.writerow({"source": c.source, "model_id": c.model_id, "lineage": c.lineage, "eligible_screen_ids": ";".join(c.screen_ids), "CDKN2A_matrix_value": c.matrix_value, "CDKN2A_status": c.status})
...
def run(args: argparse.Namespace, stage: Path) -> dict[str, object]:
    ...
    return {"experiment_id": EXPERIMENT_ID, "status": "FEASIBILITY_ONLY_NOMINAL_GATES_PASS" if nominal_pass else "FEASIBILITY_ONLY_NOMINAL_GATE_FAILURE", "analysis_label": "FEASIBILITY_ONLY", "confirmatory_claim": False, "analysis_type": "preregistered_source_specific_lineage_stratified_cdkn2a_tyms_replication", "claim_eligibility": {"primary_confirmatory": False, "design_sensitivity_label": "FEASIBILITY_ONLY", "nominal_primary_gates_pass": nominal_pass}, "context_receipt": context_receipt, "design_sensitivity": design_receipt, "pre_endpoint_receipt": pre_receipt, "endpoint_receipt": endpoint_receipt, "primary": results, "inference_receipt": {"design_seeds": DESIGN_SEEDS, "inference_seeds": INFERENCE_SEEDS, "permutations": engine.PERMUTATIONS, "bootstraps": engine.BOOTSTRAPS, "bootstrap_interval": "percentile_95_linear_quantile", "unit": "collapsed_source_model_id", "cross_source_raw_score_comparison": False}, "implementation_receipt": {"manifest_path": args.manifest_file, "required_base_commit": manifest["implementation_boundary"]["required_base_commit"], "implementation_commit": manifest["implementation_boundary"]["implementation_commit"], "implementation_module": manifest["implementation_boundary"]["implementation_module"], "engine_module": manifest["implementation_boundary"]["engine_module"], "uv_lock_sha256": manifest["uv_lock_sha256"]}, "artifact_receipt_sha256": {name: sha256(stage / name) for name in ("context_ledger.csv", "design_sensitivity.csv", "endpoint_scores.csv", "inference.csv")} | {"summary.json": ""}, "overall_pass": False, "claim_boundary": "damaging-matrix CDKN2A status association with source-specific TYMS dependency in frozen 23Q4 screen cohorts; no deletion, biallelic functional-loss, TYMP-high, treatment, clinical, or confirmatory claim"}
...
def validate_staged(stage: Path, result: dict[str, object]) -> None:
    if {p.name for p in stage.iterdir() if p.is_file()} != EXPECTED_RESULT_FILES:
        raise IntegrityError("EXP018 staged file set drift")
    ...
    if result["analysis_label"] != "FEASIBILITY_ONLY" or result["confirmatory_claim"] is not False or result["overall_pass"] is not False or result["artifact_receipt_sha256"]["summary.json"] != engine.summary_digest(result):
        raise IntegrityError("terminal contract/digest drift")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--manifest-file", default="experiments/EXP-20260822-018/manifest.json")
    parser.add_argument("--results-dir", default="experiments/EXP-20260822-018/results")
    parser.add_argument("--error-receipt", default="experiments/EXP-20260822-018/error_receipt.json")
    parser.add_argument("--pre-endpoint-receipt", default="experiments/EXP-20260822-018/pre_endpoint_receipt.json")
    return parser
