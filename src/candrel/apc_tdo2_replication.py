...
EXPERIMENT_ID = "EXP-20260822-023"
PAIR_ID = "APC_damaging_to_TDO2"
STATUS_COLUMN = "APC (324)"
TARGET_COLUMN = "TDO2 (6999)"
SOURCES = ("Avana", "KY")
LOADER_EXPOSED = "matrix_intact"
LOADER_REFERENCE = "damaging"
ANALYSIS_EXPOSED = "damaging"
ANALYSIS_REFERENCE = "matrix_intact"
EXPECTED_MODULE_PATH = "src/candrel/apc_tdo2_replication.py"
ENGINE_MODULE_PATH = "src/candrel/tp53_mdm2_replication.py"
PROJECT_PATH = "pyproject.toml"
SELECTION_SEAL_PATH = "experiments/EXP-20260822-023/selection_seal.json"
REQUIRED_BASE_COMMIT = "7e77bd2c5c8936cc72d3af1fdd39c433a1ae408c"
EXPECTED_HASHES = {
...
EXPECTED_STATUS_COUNTS = {
    "Avana": {ANALYSIS_EXPOSED: 54, ANALYSIS_REFERENCE: 921},
    "KY": {ANALYSIS_EXPOSED: 30, ANALYSIS_REFERENCE: 285},
}
EXPECTED_MIXED_LINEAGES = {"Avana": 11, "KY": 5}
DESIGN_SEEDS = {"Avana": 20262300, "KY": 20262400}
INFERENCE_SEEDS = {"Avana": 20272300, "KY": 20272400}
EXPECTED_ROSTER_SHA256 = "64d3f95cf8bac59c1b7293b464c8cbe9133441a94c9e44897529540c9c58fb8d"
EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256 = "c2286274aea239574d5f6cd56894992ef0c29fe5ac88335874e64fe50c1f5bab"
...
def verify_implementation_boundary(manifest_path: Path) -> dict[str, object]:
    try:
        ...
        if manifest["experiment_id"] != EXPERIMENT_ID or manifest["entrypoint"] != ".venv/bin/python -m candrel.apc_tdo2_replication":
        ...
        if manifest["selection_seal"]["sealed_before_endpoint_access"] is not True or manifest["selection_seal"]["protocol_commit"] != "2b5ae5ae33c5cb999f82458e53397841cc6879ff":
            raise ValueError("selection seal contract drift")
        expected_design = {"mean_shift": engine.NORMAL_MEAN_SHIFT, "null_permutations": engine.PERMUTATIONS, "alternative_simulations": engine.DESIGN_SIMULATIONS, "planning_power_seeds": DESIGN_SEEDS, "expected_critical_delta": {"Avana": -0.19735099337748344, "KY": -0.3103448275862069}, "expected_power": {"Avana": 0.5184, "KY": 0.2861}, "minimum_power_for_confirmatory_label": engine.MIN_CONFIRMATORY_POWER, "frozen_label": "FEASIBILITY_ONLY"}
...
def write_context_ledger(path: Path, contexts: Sequence[Context]) -> None:
    fields = ["source", "model_id", "lineage", "eligible_screen_ids", "APC_matrix_value", "APC_status"]
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for c in contexts:
            writer.writerow({"source": c.source, "model_id": c.model_id, "lineage": c.lineage, "eligible_screen_ids": ";".join(c.screen_ids), "APC_matrix_value": c.matrix_value, "APC_status": c.status})
...
def run(args: argparse.Namespace, stage: Path) -> dict[str, object]:
    ...
    return {"experiment_id": EXPERIMENT_ID, "status": "FEASIBILITY_ONLY_NOMINAL_GATES_PASS" if nominal_pass else "FEASIBILITY_ONLY_NOMINAL_GATE_FAILURE", "analysis_label": "FEASIBILITY_ONLY", "confirmatory_claim": False, "analysis_type": "preregistered_source_specific_lineage_stratified_apc_tdo2_replication", "claim_eligibility": {"primary_confirmatory": False, "design_sensitivity_label": "FEASIBILITY_ONLY", "nominal_primary_gates_pass": nominal_pass}, "context_receipt": context_receipt, "design_sensitivity": design_receipt, "pre_endpoint_receipt": pre_receipt, "endpoint_receipt": endpoint_receipt, "primary": results, "inference_receipt": {"design_seeds": DESIGN_SEEDS, "inference_seeds": INFERENCE_SEEDS, "permutations": engine.PERMUTATIONS, "bootstraps": engine.BOOTSTRAPS, "bootstrap_interval": "percentile_95_linear_quantile", "unit": "collapsed_source_model_id", "cross_source_raw_score_comparison": False}, "implementation_receipt": {"manifest_path": args.manifest_file, "required_base_commit": manifest["implementation_boundary"]["required_base_commit"], "implementation_commit": manifest["implementation_boundary"]["implementation_commit"], "implementation_module": manifest["implementation_boundary"]["implementation_module"], "engine_module": manifest["implementation_boundary"]["engine_module"], "uv_lock_sha256": manifest["uv_lock_sha256"]}, "artifact_receipt_sha256": {name: sha256(stage / name) for name in ("context_ledger.csv", "design_sensitivity.csv", "endpoint_scores.csv", "inference.csv")} | {"summary.json": ""}, "overall_pass": False, "claim_boundary": "damaging-matrix APC status association with source-specific TDO2 dependency in frozen 23Q4 screen cohorts; no functional-APC, APC-null, WNT-causal, TDO2-inhibitor, treatment, clinical, or confirmatory claim"}
...
def validate_staged(stage: Path, result: dict[str, object]) -> None:
    if {p.name for p in stage.iterdir() if p.is_file()} != EXPECTED_RESULT_FILES:
        raise IntegrityError("EXP023 staged file set drift")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--manifest-file", default="experiments/EXP-20260822-023/manifest.json")
    parser.add_argument("--results-dir", default="experiments/EXP-20260822-023/results")
    parser.add_argument("--error-receipt", default="experiments/EXP-20260822-023/error_receipt.json")
    parser.add_argument("--pre-endpoint-receipt", default="experiments/EXP-20260822-023/pre_endpoint_receipt.json")
    return parser
