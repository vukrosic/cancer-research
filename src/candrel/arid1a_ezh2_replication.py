"""Bounded ARID1A-proxy to EZH2 dependency transportability replication."""
...
EXPERIMENT_ID = "EXP-20260822-027"
PAIR_ID = "ARID1A_damaging_to_EZH2"
STATUS_COLUMN = "ARID1A (8289)"
TARGET_COLUMN = "EZH2 (2146)"
SOURCES = ("Avana", "KY")
LOADER_EXPOSED = "matrix_intact"
LOADER_REFERENCE = "damaging"
ANALYSIS_EXPOSED = "damaging"
ANALYSIS_REFERENCE = "matrix_intact"
EXPECTED_MODULE_PATH = "src/candrel/arid1a_ezh2_replication.py"
ENGINE_MODULE_PATH = "src/candrel/tp53_mdm2_replication.py"
PROJECT_PATH = "pyproject.toml"
EXPECTED_PROTOCOL_COMMIT = "756a928"
REQUIRED_BASE_COMMIT = "756a928"
EXPECTED_HASHES = {
...
EXPECTED_STATUS_COUNTS = {
    "Avana": {ANALYSIS_EXPOSED: 101, ANALYSIS_REFERENCE: 874},
    "KY": {ANALYSIS_EXPOSED: 43, ANALYSIS_REFERENCE: 272},
}
EXPECTED_MIXED_LINEAGES = {"Avana": 19, "KY": 11}
DESIGN_SEEDS = {"Avana": 20262700, "KY": 20262800}
INFERENCE_SEEDS = {"Avana": 20272700, "KY": 20272800}
EXPECTED_ROSTER_SHA256 = "62f8bf69649eb375de12daed222a50a4bc3b3df39c40f0e614845fbab12ab9ed"
EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256 = "0b38286f8fe6ad583090e1a0da8be4cd9aa12edc370c2b8b5110dba973e633d6"
...
def verify_implementation_boundary(manifest_path: Path) -> dict[str, object]:
    try:
        ...
        if manifest["experiment_id"] != EXPERIMENT_ID or manifest["entrypoint"] != ".venv/bin/python -m candrel.arid1a_ezh2_replication":
        ...
        if manifest["selection_seal"]["sealed_before_endpoint_access"] is not True or manifest["selection_seal"]["protocol_commit"] != EXPECTED_PROTOCOL_COMMIT:
            raise ValueError("selection seal contract drift")
        expected_design = {"mean_shift": engine.NORMAL_MEAN_SHIFT, "null_permutations": engine.PERMUTATIONS, "alternative_simulations": engine.DESIGN_SIMULATIONS, "planning_power_seeds": DESIGN_SEEDS, "expected_critical_delta": {"Avana": -0.1213903743315508, "KY": -0.18319719953325556}, "expected_power": {"Avana": 0.8621, "KY": 0.576}, "minimum_power_for_confirmatory_label": engine.MIN_CONFIRMATORY_POWER, "frozen_label": "FEASIBILITY_ONLY"}
...
def write_context_ledger(path: Path, contexts: Sequence[Context]) -> None:
    fields = ["source", "model_id", "lineage", "eligible_screen_ids", "ARID1A_matrix_value", "ARID1A_status"]
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for context in contexts:
            writer.writerow({"source": context.source, "model_id": context.model_id, "lineage": context.lineage, "eligible_screen_ids": ";".join(context.screen_ids), "ARID1A_matrix_value": context.matrix_value, "ARID1A_status": context.status})
...
def validate_staged(stage: Path, result: dict[str, object]) -> None:
    if {path.name for path in stage.iterdir() if path.is_file()} != EXPECTED_RESULT_FILES:
        raise IntegrityError("EXP027 staged file set drift")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--manifest-file", default="experiments/EXP-20260822-027/manifest.json")
    parser.add_argument("--results-dir", default="experiments/EXP-20260822-027/results")
    parser.add_argument("--error-receipt", default="experiments/EXP-20260822-027/error_receipt.json")
    parser.add_argument("--pre-endpoint-receipt", default="experiments/EXP-20260822-027/pre_endpoint_receipt.json")
    return parser
