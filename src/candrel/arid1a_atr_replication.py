"""Bounded ARID1A-proxy to ATR dependency transport audit."""
...
EXPECTED_STATUS_COUNTS = {"Avana": {ANALYSIS_EXPOSED: 101, ANALYSIS_REFERENCE: 874}, "KY": {ANALYSIS_EXPOSED: 43, ANALYSIS_REFERENCE: 272}}
EXPECTED_MIXED_LINEAGES = {"Avana": 19, "KY": 11}
DESIGN_SEEDS = {"Avana": 20263300, "KY": 20263400}
INFERENCE_SEEDS = {"Avana": 20273300, "KY": 20273400}
EXPECTED_ROSTER_SHA256 = "62f8bf69649eb375de12daed222a50a4bc3b3df39c40f0e614845fbab12ab9ed"
EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256 = "0243c5b79cd3b918e22fcbd371c79b851240ea131ed99b3a933a65c221433b71"
...
def verify_implementation_boundary(manifest_path: Path) -> dict[str, object]:
    try:
        ...
        if manifest["experiment_id"] != EXPERIMENT_ID or manifest["entrypoint"] != ".venv/bin/python -m candrel.arid1a_atr_replication":
...
def run(args: argparse.Namespace, stage: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise T0Stop(engine.classify_context_stop(exc), False, str(exc)) from exc
...
def validate_staged(stage: Path, result: dict[str, object]) -> None:
    if {path.name for path in stage.iterdir() if path.is_file()} != EXPECTED_RESULT_FILES:
        raise IntegrityError("EXP033 staged file set drift")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--manifest-file", default="experiments/EXP-20260822-033/manifest.json")
    parser.add_argument("--results-dir", default="experiments/EXP-20260822-033/results")
    parser.add_argument("--error-receipt", default="experiments/EXP-20260822-033/error_receipt.json")
    parser.add_argument("--pre-endpoint-receipt", default="experiments/EXP-20260822-033/pre_endpoint_receipt.json")
    return parser
