"""Bounded composite BRCA1/2-proxy to POLQ dependency transport audit."""
...
EXPERIMENT_ID = "EXP-20260822-029"
PAIR_ID = "BRCA1_or_BRCA2_damaging_to_POLQ"
STATUS_COLUMNS = ("BRCA1 (672)", "BRCA2 (675)")
STATUS_LABEL = "BRCA1_or_BRCA2_composite"
TARGET_COLUMN = "POLQ (10721)"
SOURCES = ("Avana", "KY")
LOADER_EXPOSED = "matrix_intact"
LOADER_REFERENCE = "damaging"
ANALYSIS_EXPOSED = "damaging"
ANALYSIS_REFERENCE = "matrix_intact"
EXPECTED_MODULE_PATH = "src/candrel/brca12_polq_replication.py"
ENGINE_MODULE_PATH = "src/candrel/tp53_mdm2_replication.py"
PROJECT_PATH = "pyproject.toml"
REQUIRED_BASE_COMMIT = "43bb802"
EXPECTED_HASHES = {
...
EXPECTED_STATUS_COUNTS = {
    "Avana": {ANALYSIS_EXPOSED: 46, ANALYSIS_REFERENCE: 929},
    "KY": {ANALYSIS_EXPOSED: 25, ANALYSIS_REFERENCE: 290},
}
EXPECTED_MIXED_LINEAGES = {"Avana": 16, "KY": 8}
DESIGN_SEEDS = {"Avana": 20262900, "KY": 20263000}
INFERENCE_SEEDS = {"Avana": 20272900, "KY": 20273000}
EXPECTED_ROSTER_SHA256 = "058842fec2d6750661de8d3109950f7e6376456baa564b5bd3cb49697fa3d083"
EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256 = "2f9531af2d7f80028f7bc5b3e224cc69825d26bf5dfe9d4a518733437a602af7"
...
def verify_implementation_boundary(manifest_path: Path) -> dict[str, object]:
    try:
        ...
        if manifest["experiment_id"] != EXPERIMENT_ID or manifest["entrypoint"] != ".venv/bin/python -m candrel.brca12_polq_replication":
...
def _load_composite_status_matrix(path: Path, model_ids: set[str]) -> tuple[dict[str, int], str]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        if header is None or not header or header[0] != "" or any(header.count(column) != 1 for column in STATUS_COLUMNS):
            raise IntegrityError("BRCA1/BRCA2 matrix header drift")
        ...
        for row in reader:
            ...
            if model_id in model_ids:
                ...
                values = [engine.parse_matrix_value(row[index], (model_id, column)) for index, column in zip(indices, STATUS_COLUMNS)]
                statuses[model_id] = int(any(value in {1, 2} for value in values))
    if set(statuses) != model_ids:
        raise IntegrityError(f"BRCA1/BRCA2 matrix coverage drift: expected {len(model_ids)}, got {len(statuses)}")
...
def load_context(qc_path: Path, screen_map_path: Path, model_path: Path, damaging_path: Path):
    with _configured(LOADER_EXPOSED, LOADER_REFERENCE):
        original_loader = engine.load_status_matrix
        engine.load_status_matrix = _load_composite_status_matrix
        try:
            contexts, receipt = engine.load_context(qc_path, screen_map_path, model_path, damaging_path)
        finally:
            engine.load_status_matrix = original_loader
        receipt["status_column"] = STATUS_LABEL
        receipt["status_columns"] = list(STATUS_COLUMNS)
        receipt["composite_rule"] = "damaging if either BRCA1 or BRCA2 matrix value is 1 or 2; matrix_intact if both values are 0"
...
def write_context_ledger(path: Path, contexts: Sequence[Context]) -> None:
    fields = ["source", "model_id", "lineage", "eligible_screen_ids", "BRCA1_or_BRCA2_composite_matrix_value", "BRCA1_or_BRCA2_status"]
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for context in contexts:
            writer.writerow({"source": context.source, "model_id": context.model_id, "lineage": context.lineage, "eligible_screen_ids": ";".join(context.screen_ids), "BRCA1_or_BRCA2_composite_matrix_value": context.matrix_value, "BRCA1_or_BRCA2_status": context.status})


def classify_context_stop(error: IntegrityError) -> str:
    message = str(error).lower()
    if "non-numeric value" in message or "non-finite value" in message or "matrix coverage" in message:
...
def validate_staged(stage: Path, result: dict[str, object]) -> None:
    if {path.name for path in stage.iterdir() if path.is_file()} != EXPECTED_RESULT_FILES:
        raise IntegrityError("EXP029 staged file set drift")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--manifest-file", default="experiments/EXP-20260822-029/manifest.json")
    parser.add_argument("--results-dir", default="experiments/EXP-20260822-029/results")
    parser.add_argument("--error-receipt", default="experiments/EXP-20260822-029/error_receipt.json")
    parser.add_argument("--pre-endpoint-receipt", default="experiments/EXP-20260822-029/pre_endpoint_receipt.json")
    return parser
