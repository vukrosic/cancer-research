...
EXPERIMENT_ID = "EXP-20260822-021"
PAIR_ID = "NF1_damaging_to_PTPN11"
STATUS_COLUMN = "NF1 (4763)"
TARGET_COLUMN = "PTPN11 (5781)"
SOURCES = ("Avana", "KY")
LOADER_EXPOSED = "matrix_intact"
LOADER_REFERENCE = "damaging"
ANALYSIS_EXPOSED = "damaging"
ANALYSIS_REFERENCE = "matrix_intact"
EXPECTED_MODULE_PATH = "src/candrel/nf1_ptpn11_replication.py"
ENGINE_MODULE_PATH = "src/candrel/tp53_mdm2_replication.py"
PROJECT_PATH = "pyproject.toml"
SELECTION_SEAL_PATH = "experiments/EXP-20260822-021/selection_seal.json"
REQUIRED_BASE_COMMIT = "bd8eeae757dbc0221727202ed67b7b0936f1cdd9"
EXPECTED_HASHES = {
...
EXPECTED_STATUS_COUNTS = {
    "Avana": {ANALYSIS_EXPOSED: 57, ANALYSIS_REFERENCE: 918},
    "KY": {ANALYSIS_EXPOSED: 21, ANALYSIS_REFERENCE: 294},
}
EXPECTED_MIXED_LINEAGES = {"Avana": 15, "KY": 9}
DESIGN_SEEDS = {"Avana": 20262100, "KY": 20262200}
INFERENCE_SEEDS = {"Avana": 20272100, "KY": 20272200}
EXPECTED_ROSTER_SHA256 = "8c3229c5925e533688a9efb8979700d1f2d379a760d0672544a61073a8bfc375"
EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256 = "fdd988807837eb4dfb4d4085636508566cdc7a005812a3550007b51d66bd233e"
...
def verify_implementation_boundary(manifest_path: Path) -> dict[str, object]:
    try:
        ...
        if manifest["experiment_id"] != EXPERIMENT_ID or manifest["entrypoint"] != ".venv/bin/python -m candrel.nf1_ptpn11_replication":
        ...
        if manifest["selection_seal"]["sealed_before_endpoint_access"] is not True or manifest["selection_seal"]["protocol_commit"] != "ad16b66838d3e8413ce10b8f73f3a50cc093fd40":
            raise ValueError("selection seal contract drift")
        expected_design = {"mean_shift": engine.NORMAL_MEAN_SHIFT, "null_permutations": engine.PERMUTATIONS, "alternative_simulations": engine.DESIGN_SIMULATIONS, "planning_power_seeds": DESIGN_SEEDS, "expected_critical_delta": {"Avana": -0.14961776483436476, "KY": -0.23846153846153847}, "expected_power": {"Avana": 0.7173, "KY": 0.4028}, "minimum_power_for_confirmatory_label": engine.MIN_CONFIRMATORY_POWER, "frozen_label": "FEASIBILITY_ONLY"}
...
def write_context_ledger(path: Path, contexts: Sequence[Context]) -> None:
    fields = ["source", "model_id", "lineage", "eligible_screen_ids", "NF1_matrix_value", "NF1_status"]
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for c in contexts:
            writer.writerow({"source": c.source, "model_id": c.model_id, "lineage": c.lineage, "eligible_screen_ids": ";".join(c.screen_ids), "NF1_matrix_value": c.matrix_value, "NF1_status": c.status})
...
def validate_staged(stage: Path, result: dict[str, object]) -> None:
    if {p.name for p in stage.iterdir() if p.is_file()} != EXPECTED_RESULT_FILES:
        raise IntegrityError("EXP021 staged file set drift")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--manifest-file", default="experiments/EXP-20260822-021/manifest.json")
    parser.add_argument("--results-dir", default="experiments/EXP-20260822-021/results")
    parser.add_argument("--error-receipt", default="experiments/EXP-20260822-021/error_receipt.json")
    parser.add_argument("--pre-endpoint-receipt", default="experiments/EXP-20260822-021/pre_endpoint_receipt.json")
    return parser
