...
EXPERIMENT_ID = "EXP-20260822-024"
PAIR_ID = "KMT2D_damaging_to_KMT2C"
STATUS_COLUMN = "KMT2D (8085)"
TARGET_COLUMN = "KMT2C (58508)"
SOURCES = ("Avana", "KY")
LOADER_EXPOSED = "matrix_intact"
LOADER_REFERENCE = "damaging"
ANALYSIS_EXPOSED = "damaging"
ANALYSIS_REFERENCE = "matrix_intact"
EXPECTED_MODULE_PATH = "src/candrel/kmt2d_kmt2c_replication.py"
ENGINE_MODULE_PATH = "src/candrel/tp53_mdm2_replication.py"
PROJECT_PATH = "pyproject.toml"
SELECTION_SEAL_PATH = "experiments/EXP-20260822-024/selection_seal.json"
REQUIRED_BASE_COMMIT = "311d6b212f9c3c7564e4279ee41fbd59eab61f3e"
EXPECTED_HASHES = {
...
EXPECTED_STATUS_COUNTS = {
    "Avana": {ANALYSIS_EXPOSED: 86, ANALYSIS_REFERENCE: 889},
    "KY": {ANALYSIS_EXPOSED: 33, ANALYSIS_REFERENCE: 282},
}
EXPECTED_MIXED_LINEAGES = {"Avana": 17, "KY": 11}
DESIGN_SEEDS = {"Avana": 20262400, "KY": 20262500}
INFERENCE_SEEDS = {"Avana": 20272400, "KY": 20272500}
EXPECTED_ROSTER_SHA256 = "e8f440a118065e4fc23535dca196c03b8c9a08b9a4f4348516f214d19b2e9164"
EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256 = "dde5a09e73c1dd6744704a67ad5754fccfc8bee95bd4bf383f670e3799682263"
...
def verify_implementation_boundary(manifest_path: Path) -> dict[str, object]:
    try:
        ...
        if manifest["experiment_id"] != EXPERIMENT_ID or manifest["entrypoint"] != ".venv/bin/python -m candrel.kmt2d_kmt2c_replication":
        ...
        if manifest["selection_seal"]["sealed_before_endpoint_access"] is not True or manifest["selection_seal"]["protocol_commit"] != "5af7d8cf583abf46ad0a4ca3e598519020d70163":
            raise ValueError("selection seal contract drift")
        expected_design = {"mean_shift": engine.NORMAL_MEAN_SHIFT, "null_permutations": engine.PERMUTATIONS, "alternative_simulations": engine.DESIGN_SIMULATIONS, "planning_power_seeds": DESIGN_SEEDS, "expected_critical_delta": {"Avana": -0.12622333751568382, "KY": -0.1994645247657296}, "expected_power": {"Avana": 0.8433, "KY": 0.512}, "minimum_power_for_confirmatory_label": engine.MIN_CONFIRMATORY_POWER, "frozen_label": "FEASIBILITY_ONLY"}
        ...
        expected_modules = {EXPECTED_MODULE_PATH, ENGINE_MODULE_PATH, PROJECT_PATH}
        if {item["path"] for item in boundary["modules"]} != expected_modules:
...
def write_context_ledger(path: Path, contexts: Sequence[Context]) -> None:
    fields = ["source", "model_id", "lineage", "eligible_screen_ids", "KMT2D_matrix_value", "KMT2D_status"]
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for c in contexts:
            writer.writerow({"source": c.source, "model_id": c.model_id, "lineage": c.lineage, "eligible_screen_ids": ";".join(c.screen_ids), "KMT2D_matrix_value": c.matrix_value, "KMT2D_status": c.status})
...
def validate_staged(stage: Path, result: dict[str, object]) -> None:
    if {p.name for p in stage.iterdir() if p.is_file()} != EXPECTED_RESULT_FILES:
        raise IntegrityError("EXP024 staged file set drift")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--manifest-file", default="experiments/EXP-20260822-024/manifest.json")
    parser.add_argument("--results-dir", default="experiments/EXP-20260822-024/results")
    parser.add_argument("--error-receipt", default="experiments/EXP-20260822-024/error_receipt.json")
    parser.add_argument("--pre-endpoint-receipt", default="experiments/EXP-20260822-024/pre_endpoint_receipt.json")
    return parser
