"""Bounded TP53-proxy to ENDOD1 dependency transport audit."""
...
from . import arid1a_atr_replication as template


EXPERIMENT_ID = "EXP-20260822-035"
PAIR_ID = "TP53_damaging_to_ENDOD1"
STATUS_COLUMN = "TP53 (7157)"
TARGET_COLUMN = "ENDOD1 (23052)"
SOURCES = ("Avana", "KY")
ANALYSIS_EXPOSED = "damaging"
ANALYSIS_REFERENCE = "matrix_intact"
EXPECTED_MODULE_PATH = "src/candrel/tp53_endod1_replication.py"
BASE_MODULE_PATH = "src/candrel/arid1a_atr_replication.py"
ENGINE_MODULE_PATH = "src/candrel/tp53_mdm2_replication.py"
PROJECT_PATH = "pyproject.toml"
REQUIRED_BASE_COMMIT = "3cffc90"
EXPECTED_HASHES = template.EXPECTED_HASHES.copy()
...
DESIGN_SEEDS = {"Avana": 20263500, "KY": 20263600}
INFERENCE_SEEDS = {"Avana": 20273500, "KY": 20273600}
EXPECTED_ROSTER_SHA256 = "61060e6ef0c24ad1bb3acc2fbe75e9ad5f8908df505d20290cbab2189557b376"
EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256 = "464cd2d306a60d6862f9fcfc0bd321c27b0e7145f6226912a86fb0a32076a823"
EXPECTED_RESULT_FILES = {"context_ledger.csv", "design_sensitivity.csv", "endpoint_scores.csv", "inference.csv", "summary.json"}

IntegrityError = template.IntegrityError
T0Stop = template.T0Stop
Context = template.Context
engine = template.engine

_TEMPLATE_KEYS = (
    "EXPERIMENT_ID", "PAIR_ID", "STATUS_COLUMN", "TARGET_COLUMN", "EXPECTED_HASHES",
    "EXPECTED_SOURCE_MODELS", "EXPECTED_STATUS_COUNTS", "EXPECTED_MIXED_LINEAGES",
    "DESIGN_SEEDS", "INFERENCE_SEEDS", "EXPECTED_ROSTER_SHA256",
    "EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256", "EXPECTED_MODULE_PATH",
    "REQUIRED_BASE_COMMIT", "EXPECTED_RESULT_FILES",
)


def _configure_template() -> None:
    template.EXPERIMENT_ID = EXPERIMENT_ID
    template.PAIR_ID = PAIR_ID
    template.STATUS_COLUMN = STATUS_COLUMN
    template.TARGET_COLUMN = TARGET_COLUMN
    template.EXPECTED_HASHES = EXPECTED_HASHES.copy()
    template.EXPECTED_SOURCE_MODELS = EXPECTED_SOURCE_MODELS.copy()
    template.EXPECTED_STATUS_COUNTS = {source: counts.copy() for source, counts in EXPECTED_STATUS_COUNTS.items()}
    template.EXPECTED_MIXED_LINEAGES = EXPECTED_MIXED_LINEAGES.copy()
    template.DESIGN_SEEDS = DESIGN_SEEDS.copy()
    template.INFERENCE_SEEDS = INFERENCE_SEEDS.copy()
    template.EXPECTED_ROSTER_SHA256 = EXPECTED_ROSTER_SHA256
    template.EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256 = EXPECTED_DESIGN_RECEIPT_NORMALIZED_SHA256
    template.EXPECTED_MODULE_PATH = EXPECTED_MODULE_PATH
    template.REQUIRED_BASE_COMMIT = REQUIRED_BASE_COMMIT
    template.EXPECTED_RESULT_FILES = EXPECTED_RESULT_FILES.copy()
...
def verify_implementation_boundary(manifest_path: Path) -> dict[str, object]:
    try:
        ...
        if manifest["experiment_id"] != EXPERIMENT_ID or manifest["entrypoint"] != ".venv/bin/python -m candrel.tp53_endod1_replication":
        ...
        expected_design = {"mean_shift": engine.NORMAL_MEAN_SHIFT, "null_permutations": engine.PERMUTATIONS, "alternative_simulations": engine.DESIGN_SIMULATIONS, "planning_power_seeds": DESIGN_SEEDS, "expected_critical_delta": {"Avana": -0.07879789321491273, "KY": -0.1447178002894356}, "expected_power": {"Avana": 0.995, "KY": 0.7417}, "minimum_power_for_confirmatory_label": engine.MIN_CONFIRMATORY_POWER, "frozen_label": "FEASIBILITY_ONLY"}
...
def write_context_ledger(path: Path, contexts: list[Context]) -> None:
...
def grouped_contexts(contexts: list[Context], source: str):
    saved = _template_state()
    _configure_template()
    try:
        return template.grouped_contexts(contexts, source)
    finally:
        for key in _TEMPLATE_KEYS:
            if key in saved:
                setattr(template, key, saved[key])
...
def verify_endpoint_hash(path: Path) -> None:
    ...
    _configure_template()
    try:
        return template.verify_endpoint_hash(path)
    finally:
        for key in _TEMPLATE_KEYS:
            if key in saved:
                setattr(template, key, saved[key])
...
def _template_state():
    return {key: getattr(template, key) for key in _TEMPLATE_KEYS if hasattr(template, key)}


def run(args: argparse.Namespace, stage: Path) -> dict[str, object]:
    saved = _template_state()
    saved_verify = template.verify_implementation_boundary
    saved_writer = template.write_context_ledger
    _configure_template()
    template.verify_implementation_boundary = verify_implementation_boundary
    template.write_context_ledger = write_context_ledger
    try:
        result = template.run(args, stage)
        result["analysis_type"] = "preregistered_source_specific_lineage_stratified_tp53_endod1_transport_replication"
        result["claim_boundary"] = "TP53 damaging-matrix proxy association with source-specific ENDOD1 dependency in frozen 23Q4 cell-line screen cohorts; no functional TP53-loss, TP53-mutation, ENDOD1-inhibitor, DNA-repair, causal, pharmacologic, treatment, clinical, or confirmatory claim"
        return result
    finally:
        template.verify_implementation_boundary = saved_verify
        template.write_context_ledger = saved_writer
        for key in _TEMPLATE_KEYS:
            if key in saved:
                setattr(template, key, saved[key])
...
def validate_staged(stage: Path, result: dict[str, object]) -> None:
    ...
    _configure_template()
    try:
        template.validate_staged(stage, result)
    finally:
        for key in _TEMPLATE_KEYS:
            if key in saved:
                setattr(template, key, saved[key])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--manifest-file", default="experiments/EXP-20260822-035/manifest.json")
    parser.add_argument("--results-dir", default="experiments/EXP-20260822-035/results")
    parser.add_argument("--error-receipt", default="experiments/EXP-20260822-035/error_receipt.json")
    parser.add_argument("--pre-endpoint-receipt", default="experiments/EXP-20260822-035/pre_endpoint_receipt.json")
    return parser
